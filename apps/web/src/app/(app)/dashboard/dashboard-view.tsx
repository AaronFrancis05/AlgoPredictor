...
import { type ReactNode, Suspense, useEffect, useMemo, useSyncExternalStore } from "react";
...
import type { Entitlements, Pick, PicksDay } from "@/lib/schemas";
...
/** Day, league and sort live in the URL: a refresh, the back button or a shared link keeps them. */
function useDashboardParams() {
  const params = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();
  const raw = params.get("date") ?? "";
  const today = useToday(); // rolls over at the viewer's midnight while the page is open
  const day = DAY_RE.test(raw) ? raw : today;
  const league = params.get("league") || "all";
  const sort: Sort = params.get("sort") === "confidence" ? "confidence" : "kickoff";
...
/** ← / → move a day back or forward, unless the viewer is typing or using a form control. */
function useDayKeys(day: string, onPick: (d: string) => void) {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.altKey || e.ctrlKey || e.metaKey || e.shiftKey) return;
      const el = e.target as HTMLElement | null;
      if (el?.closest("input, textarea, select, [contenteditable='true'], [role='menu']")) return;
      if (e.key === "ArrowLeft") onPick(shift(day, -1));
      else if (e.key === "ArrowRight") onPick(shift(day, 1));
    };
...
function DashboardView() {
  ...
  // today's list refreshes each minute (cheap: unchanged lists come back as 304) and when the tab regains focus
  const q = useQuery({ ...picksQuery(day), placeholderData: keepPreviousData,
                       refetchInterval: isToday ? 60_000 : false, refetchOnWindowFocus: isToday });
  useDayKeys(day, selectDay);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex items-center gap-2 lg:w-[30rem]">
          ...
          {!isToday ? <Button variant="secondary" onClick={() => selectDay(todayLocal())}>Today</Button> : null}
        </div>
      </div>
